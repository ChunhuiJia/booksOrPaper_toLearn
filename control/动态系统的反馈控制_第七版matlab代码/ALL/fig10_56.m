%  Figure 10.56    Feedback Control of Dynamic Systems, 7e
%                        Franklin, Powell, Emami
%
% fig10_56.m is a script for disk read/write head control design
...
semilogx(w,magdb1,'LineWidth',2);
grid;
title('Fig. 10.56: Disk drive control with gain-stabilized resonance');
xlabel('\omega (rad/sec)');
ylabel('Magnitude (db)');
subplot(212);
semilogx(w,[ph(:,:); -180*ones(size(ph(:,:)))],'LineWidth',2);
grid;
xlabel('\omega (rad/sec)');
ylabel('Phase (deg)');
%Bode grid
