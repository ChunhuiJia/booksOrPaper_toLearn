%  Figure 10.08      Feedback Control of Dynamic Systems, 7e
...
subplot(211) ; loglog(w,magol2,'LineWidth',2); grid; hold on;
xlabel('\omega (rad/sec)');
ylabel('Magnitude |D_{c2}(s)G(s)|');
loglog(w,ones(size(magol2)),'g','LineWidth',2);
title('Fig. 10.8: Frequency response of low-gain satellite PD design')
phol2a=[phol2, -180*ones(size(phol2))];
subplot(212);  semilogx(w, phol2a,'LineWidth',2); grid; hold on; 
xlabel('\omega (rad/sec)');
ylabel('Phase (deg)');
%Bode grid
bodegrid;
