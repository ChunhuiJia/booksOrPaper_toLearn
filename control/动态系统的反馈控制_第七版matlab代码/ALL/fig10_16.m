%  Figure 10.16      Feedback Control of Dynamic Systems, 7e
...
subplot(211); 
loglog(w,[mag3, ones(size(mag3))],'LineWidth',2); grid;
xlabel('\omega (rad/sec)');
ylabel('Magnitude, KD_{c3}(s)Ghat(s)|');
title('Fig. 10.16: Frequency response for KD_{c3}(s)Ghat(s).')
subplot(212); 
semilogx(w,[ph3, ph180],'LineWidth',2);  
grid; 
xlabel('\omega (rad/sec)');
ylabel('Phase (deg)');
%Bode grid
