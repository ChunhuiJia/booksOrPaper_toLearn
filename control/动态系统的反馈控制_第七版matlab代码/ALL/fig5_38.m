% Figure 5.38 Feedback Control of Dynamic Systems, 7e 
...
axis([0 30 0 6]);
title('Fig. 5.38(a)  Step response of theta to 5 Deg. \theta command')
xlabel('Time  (sec)')
ylabel('\theta  (Deg)')
subplot(212),plot(t2,5*de),grid on;
axis([0 30 -.2 .2]);
title('Fig. 5.38(b) Response of elevator to 5 Deg. \theta command')
xlabel('Time  (sec)')
ylabel('\delta_e (Deg)')
subplot(111)
